            ...
            <ul className="max-h-72 overflow-y-auto">
              {items.map(n => (
                <li
                  ...
                  className={`px-4 py-3 border-b border-gray-50 last:border-0 cursor-pointer hover:bg-gray-50 transition-colors ${!n.read ? 'bg-blue-50/40' : ''}`}
                >
...
function UserMenu({ currentUser, onLogout }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-100 rounded-2xl shadow-lg z-50 py-1.5 overflow-hidden">
          ...
          <button onClick={() => { onLogout(); setOpen(false) }}
            className="w-full text-left px-4 py-2.5 text-sm text-red-500 hover:bg-gray-50 transition-colors cursor-pointer">
            Log out
          </button>
        </div>
      )}
    ...
    <nav className="flex items-center justify-between px-4 md:px-8 py-4 border-b border-gray-200 bg-white">
      ...
      <div className="hidden md:flex items-center gap-3">
        {currentUser ? (
          ...
          <>
            <Link to="/login" className="px-5 py-2 rounded-full border border-gray-300 text-sm font-medium text-gray-700 cursor-pointer hover:bg-gray-50">
              Log in
            </Link>
            <Link to="/signup" className="px-5 py-2 rounded-full bg-gray-900 text-sm font-medium text-white cursor-pointer hover:bg-gray-700">
              Sign up
            </Link>
          </>
        )}
